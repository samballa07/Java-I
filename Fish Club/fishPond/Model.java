...
public class Model {
	...
	public Model(Model other) {
		...
		for(Fish currFish: other.fish) {
			fish.add(new Fish(currFish));
		}
		
		for(Plant currPlant: other.plants) {
			plants.add(new Plant(currPlant));
		}
	}
	...
	public static void fishEatPlant(Fish f, Plant p) {
		...
		if (Plant.PLANT_BITE_SIZE < p.getSize()) {
			portion = Plant.PLANT_BITE_SIZE;
		}else {
			portion = p.getSize();
		}
		f.eat(portion);
		p.removeBite(portion);
		...
		for (Fish f : fish) {
			if(f.isAlive()) {
				if (fishIsSurroundedByRocks(f)) {
					continue;
				} else if (f.getDirection() != Fish.UP && 
						f.getDirection() != Fish.DOWN &&
						f.getDirection() != Fish.RIGHT && 
						f.getDirection() != Fish.LEFT) {
	...
	public void moveFish() {
		for(Fish f: fish) {
			if (f.isAlive()) {
				...
				for(Fish other : fish) {
					if(other.getCol() == f.getCol() && 
					   other.getRow() == f.getRow() && !other.equals(f)) {
						f.fight(other);
					}
				}
			}
	...
	public void addPlant(Plant p) {
		...
		for(Plant plant : plants) {
			if(p.getRow() == plant.getRow() && p.getCol() == plant.getCol() &&
				!p.equals(plant)) {
				throw new IllegalPlantPositionException
